...
class KomodoSpeech:
    ...
    def __init__(self, voice='voice_cmu_us_rms_arctic_clunits'):
        ...
        rospy.init_node("KomodoSpeech", anonymous=False, log_level=rospy_debug_level)
        ...
        self.sayResultPub = rospy.Publisher(self.TOPIC_PREFIX + "say_result", KomodoSpeechSayResult, queue_size=2)
        self.speechRecPub = rospy.Publisher(self.TOPIC_PREFIX + "speech_rec_result", KomodoSpeechRecResult,
                                            queue_size=2)

        self.sayCommandSub = \
            rospy.Subscriber(self.TOPIC_PREFIX + "say_command", KomodoSpeechSayCommand, self.say_cb)
        self.speechRecSub = \
            rospy.Subscriber(self.TOPIC_PREFIX + "rec_command", KomodoSpeechRecCommand, self.todo_implement)

        self.isProcessingSrv = \
            rospy.Service(self.TOPIC_PREFIX + "isProcessing", KomodoSpeechProcessing, self.is_running_cb)

        rospy.loginfo("{}: Speech system ready".format(fname))

    def todo_implement(self, data):
        return 1 / 0

    def is_running_cb(self, data):
        return KomodoSpeechProcessingResponse(self.isProcessing)

    def say_cb(self, say_command_data):
        fname = "{}::{}".format(self.NODE_NAME, self.bgr_listener_cb.__name__)
        self.isProcessing = True

        if (say_command_data.text_to_say == ""):
            rospy.logdebug("{}: Shutting up".format(fname))
            self.soundClient.stopAll()
        else:
            rospy.logdebug("{}: Saying \"{}\"...".format(fname, say_command_data.text_to_say))
            self.soundClient.say(say_command_data.text_to_say, self.soundClientVoice)

        self.isProcessing = False

        say_result = KomodoSpeechSayResult()
        say_result.success = True
    ...
    def capture_image(self, mock_file=None):
        ...
        rgb_listener = None  # rospy.Subscriber("/komodo_1/komodo_1_Asus_Camera/rgb/image_raw",
        ...
        depth_listener = None  # rospy.Subscriber("/komodo_1/komodo_1_Asus_Camera/depth/image_raw",
    ...
    def room_scan(self):
        ...
        if (best_match is None):
            ...
            match_cost = 1000  # SCAN_FAILED_COST
        else:
    ...
    def publish_scan_result(self, match_cost, matched_contour, match_frame, match_frame_idx=0):
        ...
        scan_result = None  # RoomScannerResult()

        if (match_cost >= 1000):  # SCAN_FAILED_COST):
            match_angle = 0
            match_distance = -1
        else:
            ...
            match_angle = 0  # self.camera_horizontal_arc * match_frame_idx + match_frame_angle
        ...
        self.sayResultPub.publish(scan_result)
